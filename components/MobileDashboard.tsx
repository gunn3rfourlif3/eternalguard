  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <nav className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[90%] bg-white/80 backdrop-blur-2xl border border-slate-200/50 rounded-full p-2 flex justify-around shadow-xl z-50">
        <button onClick={() => setActiveTab("home")} className={`p-4 rounded-2xl ${activeTab === "home" ? "text-eternal-gold bg-eternal-gold/5 shadow-sm" : "text-slate-300"}`}><ShieldCheck size={24} /></button>
        <button onClick={() => setActiveTab("vault")} className={`p-4 rounded-2xl ${activeTab === "vault" ? "text-eternal-gold bg-eternal-gold/5 shadow-sm" : "text-slate-300"}`}><Lock size={24} /></button>
